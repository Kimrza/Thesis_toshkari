...
| **RES-02** | `team-practices.md` § Testing Posture is stale on two figures: it defines Phase 1's acceptance set as WS-09 through WS-20, omitting FR-WS-4's WS-01 exception; and it states 17 §12-tree test modules where the amended tree and REQ-ENG-4 both now read 19 ⚠ **CORRECTED 2026-08-28 — this entry was itself stale on the very figure it exists to track. The derived count is 21, not 19.** Derived by enumerating §12s `tests/` tree and printed before assertion; set-differenced against `team.md`s affirmed **17** as **+4 / −0**. The **19** written here was carried from an intermediate revision and never re-derived. **Annotated in place on the project decision owners express approval, 2026-08-28** (`GOV-2026-08-28-FD-01` follow-on), which `governance/CHANGE_RECORD_PROCEDURE.md` requires for a completed-stage artifact; the superseded **19** is preserved above rather than overwritten. The obligation this entry carries to the practices-affirmation gate is **unchanged in kind and corrected in value**: `team.md` must move **17 → 21** | The practices-affirmation gate owner | Next authorized practices-affirmation gate | Deferred. `org.md` reserves that file for that gate, so this plan does not edit it. **The Q11 answer adds a third item to flag there**: the narrower reading of TC-06's "test suite" |
