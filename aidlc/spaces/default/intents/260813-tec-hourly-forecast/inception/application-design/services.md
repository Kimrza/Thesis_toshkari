...
Q5 = C. **Two artifacts, one authoritative** *(three between 2026-08-24 and 2026-08-25; see
both notes below — the ledger row was added on 2026-08-24 and removed on 2026-08-25 when
Amendment C was declined as drafted)*:
...
> ## ⛔ THE RELEASE-HISTORY LEDGER ROW WAS REMOVED 2026-08-25
>
> **Amendment C was DECLINED AS DRAFTED** by the project decision owner on 2026-08-25,
> reversing its 2026-08-24 approval recorded in the note below. `artifacts/registry/release_history.jsonl`
> is **not created**, and `foundation` derives `dataset_version` from the release `content_hash`
> instead — no ledger, no allocation step, no `ReleaseLedgerEntry`.
>
> **Superseded row, preserved verbatim:** *"| `artifacts/registry/release_history.jsonl` |
> **authoritative for release labels only** — never for release identity | `foundation`'s
> `src/data/release.py`, at each release write | durable, append-only; one row per allocated
> label carrying the label, its authoritative `content_hash`, the release path and the `run_id`
> |"*, and the superseded opening *"**Three artifacts, one authoritative** (two until
> 2026-08-24 …)"*.
>
> **Why this edit was made from a Construction stage, and on what authority.**
> `functional-design` first **reported** this site as an owed upstream correction rather than
> making it, because its scope control forbade editing an approved Inception artifact; the owner
...
> other unit referenced the ledger, so nothing further is orphaned.

> ## ⚠ THE RELEASE-HISTORY LEDGER, ADDED 2026-08-24
>
> **Superseded text, preserved:** *"Q5 = C. **Two artifacts**, one authoritative"*,
> listing only the two `experiment_registry` files.
>
> **Why it exists.** `functional-design` for `foundation` answered two questions the
> approved design had left open:
>
> - **Q6 = D** — *"Use a content-derived SHA-256 hash as the authoritative release identity **and assign a separate monotonic, human-readable release label** for review and citation."* Chosen over option C, *"the version is derived from the manifest hash"*, which was read and declined.
> - **FU-2 = D** — *"Define a separate, foundation-owned, **durable, append-only release-history ledger** for human-readable release labels and their authoritative content hashes. Keep it separate from `experiment_registry.jsonl`."*
>
> A **monotonic** label requires remembering what was last allocated. That is durable
> state, and it is why a directory scan will not do: delete a release directory and a
> rebuilt index forgets its label, so the next allocation reuses it. A ledger cannot
> forget.
>
> **Kept deliberately separate** from `experiment_registry.jsonl` — FU-2=D says so
> directly. The two answer different questions: the registry records *runs*, the
> ledger records *release labels*.
>
> **It does not touch release identity.** `foundation`'s **R-11** keeps the
> **content hash** authoritative; the label is *"derived, for citation at a
> human-reviewed gate, and explicitly not authoritative"*. Every integrity guarantee
> stays hash-based, so this artifact carries the weaker identifier only.
>
...
> file-level entries** inside any `artifacts/` subdirectory. Confirming from the other
> direction, `experiment_registry.jsonl` is not named in the Technical Environment
> either; it originates in this artifact.
>
> **Stated plainly:** no *requirement* mandates a ledger — the word appears **0 times**
> in `requirements.md`. Its authority is Q6=D and FU-2=D, two approved owner answers,
> on the same footing as any other approved stage decision.
>
> Annotated in place under the `GOV-2026-08-22-INC-01` Rec 7 precedent, on the project
> decision owner's approval.
> Change record: `CR-2026-08-24-FOUNDATION-AMENDMENTS`, Amendment C.
