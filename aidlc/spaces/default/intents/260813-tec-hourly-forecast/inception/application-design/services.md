...
Q5 = C. **Three artifacts, one authoritative** *(two until 2026-08-24 — see the
...
| `artifacts/registry/release_history.jsonl` | **authoritative for release labels only** — never for release identity | `foundation`'s `src/data/release.py`, at each release write | durable, append-only; one row per allocated label carrying the label, its authoritative `content_hash`, the release path and the `run_id` |
