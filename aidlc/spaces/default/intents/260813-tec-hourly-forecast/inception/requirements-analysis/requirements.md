...
| REQ-ENG-4 | The **21** mandated test modules exist under `tests/` — **all twenty-one are enumerated in §12's tree directly**, the count re-derived from that amended tree on 2026-08-22 by listing its `test_*.py` entries and counting them, not carried from prose. The tree reached twenty-one by **four** amendments of three authorities: `test_acquisition_window.py` was **countersigned 2026-08-16** (`governance/COUNTERSIGNATURE_REQUEST_2026-08-16.md` item 1) but was not written into §12 until 2026-08-22, and `test_determinism.py` was added under **ADR-10** — both applied under `CR-2026-08-22-TE-AMEND`, taking the tree from 17 to 19; `test_prepared_target_schema.py` was then added under **BLK-05** (`CR-2026-08-22-TARGET-SCHEMA-TEST`), taking it to 20; and `test_feature_leakage_guards.py` under `CR-2026-08-22-LEAKAGE-TA`, taking it to 21 — all three 2026-08-22 acts approved by the project owner under the recorded student/supervisor authority equivalence. **Superseded text, preserved for the audit trail:** *"all nineteen are now enumerated in §12's tree directly"* and *"The tree reached nineteen by two amendments of different authority"* — both froze at the first of the three same-day amendments and were short by two. Corrected 2026-08-22 under `CR-2026-08-22-INC-CORRECTIONS` per `GOV-2026-08-22-INC-01` Rec 3. No module's presence in the tree implies it exists: **18 of the 21 are unwritten** — only `test_acquisition_window.py`, `test_phase_boundary.py` and `test_release_hashes.py` exist — and each unwritten module remains subject to G-09 and its own stage — plus the two fixture directories `tests/fixtures/plumbing_7day/` and `tests/fixtures/scientific_1month/` | Each named module is present and collectible, `test_acquisition_window.py` included, since FR-P1-01-5 and FR-WS-3 discharge onto it. **Fixture assertion data lives in `fixture_manifest.yaml` carrying all thirteen of TE §15.2's content areas** — identity, input hashes, expected schema, row-count ranges, support and missingness limits, timestamp tolerances, required outputs, expected CPU runtime range measured before freeze, and permitted floating-point tolerances — never hardcoded in test bodies. Without the tolerance areas, FR-WS-5's *"within declared tolerances"* is evaluated against tolerances no requirement requires to exist. **TE §15.4's `artifact_manifest.json` hash-listing is required as well**, and **TE §13.7's rule that a mismatch "must not silently update the expected value"** governs every fixture expectation. **D-11's pre-freeze obligation stands: ARUC's one-bin shortfall on five of seven days is explained before the plumbing fixture is frozen**, not after [origin `DATA-11`+`IMPL-5`] | [TE §12, §15.2] [practices] [countersigned 2026-08-16] | TA-09 — bounded, see § Known defects row 8 |
...
| REQ-ENG-6 | Git is initialized before any further acquisition work, on `main`, with a credential/secret deny-list in `.gitignore` (`.env`, `*.key`, `kaggle.json`, `.netrc`, `credentials*`) present **before the first commit** | `git log` exists; **a secret scan over the working tree returns clean, and the pre-existing history breach is recorded separately rather than folded into that check** — the two are different obligations and the combined form was already unsatisfiable. `notebooks/madrigal_phase1_coverage_audit.ipynb` cell 2 carried `USER_EMAIL = '<redacted-personal-email>'` in one historical commit (`33d231b`, 2026-08-17; removed from the working tree 2026-09-20, redacted here 2026-09-24 per `CHANGE_RECORD_2026-09-24_email_finding_rescoped.md` §B-1 — not "every commit": re-derived, only 2 commits touch this file and only 1 carries the literal), and thirteen committed manifests carry `user_fullname` and `user_affiliation` (investigated and closed as a non-issue, §B-2 of the same record — standard academic attribution, not an NFR-SEC-01 concern); history is not rewritable without breaking the audit-trail immutability `team.md` affirms. So: prospective cleanliness is **tested**; the historical breach is **recorded** with its chosen remediation, and the underlying conflict is recorded in § Known defects with no reading adopted. The `.gitignore` deny-list this requirement names is already present [origin `DATA-16`] | [practices] [TE §10] [NFR-SEC-01] | TA-22 |
...
### Vision §11.2 claim-boundary requirement [added 2026-08-22]

`REQ-CLAIM-01` is a **Vision §11.2 ID adopted unchanged**, not a TE §11 NFR, so
it sits in its own table rather than in the eleven-row adoption table above —
that table's count and provenance are unaffected. Added 2026-08-22 under
`CR-2026-08-22-INC-CORRECTIONS`, on the project owner's approval of
`GOV-2026-08-22-INC-01` **Recommendation 10, Option 2**. Until now this
requirement was reachable only through § Traceability and § Out of scope C, so a
G-08 reviewer looking for the claims control in the requirement tables did not
find a row. **The criterion is not new and is not restated here**: it is the one
already enumerated in § Out of scope C, referenced rather than copied, so the two
cannot drift apart.

| ID | Requirement | Pass/fail criterion | Source | Test |
|---|---|---|---|---|
| REQ-CLAIM-01 | No artifact, figure caption, table, or abstract-level statement asserts any claim outside the frozen boundary: hourly VTEC forecasting at ARUC 40/44, BSHM 32/35 and NICO 35/33, calendar year 2022, +1-hour horizon, tested on December 2022 only. The three sites are **correlated, not independent spatial samples**, and the sector name is **descriptive only**. The affirmative characterisation required by Vision §6.2 accompanies the claim boundary | **The claims-and-limitations checklist of § Out of scope C**, applied per prohibited class enumerated there (positioning-domain; all-Iran / whole-sector / arbitrary-location; multi-year or solar-cycle generalisation; commercial / user-market / UI / deployment; real-time production service; operational readiness, commercial value, positioning benefit or user demand) — each recorded as unasserted across every reported artifact. The enumeration is **maintained in § Out of scope C only**; this row cites it and must not duplicate it | [Vision §11.2, ID adopted unchanged] [Vision §4.2, §4.3, §2.5, §6.2] [D-8] [TC-17] | `UNTESTED` — `TST-CLAIMS-01` is named by Vision §11.2 but has no §16 or §19 row. **Adding a criterion is not adding an acceptance row**: creating one is a Vision §15.2 amendment and is not done here |

**Effect on the counts, derived not asserted.** Requirement-definition rows go
from 104 to **105**, matching the 105 distinct IDs every artifact already
reports, so no other count changes. The untested total stays **36** and becomes
**fully derivable from the test-row column** — previously 35 rows carried
`UNTESTED` and `REQ-CLAIM-01` had to be added by hand, which is why
§ Requirements with no testing row could not literally have recomputed all 36
from that column.
...
column, not decremented by hand.** **As of 2026-08-22 that statement holds for
all 36 entries.** It previously could not: `REQ-CLAIM-01` had no requirement row
and therefore no test-row cell, so it had to be added to this list by hand. It
gained a row in § Non-functional requirements under `GOV-2026-08-22-INC-01`
Rec 10, and the total is now reproducible in one step — 36 rows carry `UNTESTED`
in the requirement tables' test-row column.
...
| REQ-CLAIM-01 | Vision §11.2 (ID adopted unchanged), §4.2, §4.3, §2.5, §6.2 | intent § Claim boundary; D-8 | `TST-CLAIMS-01` named by Vision §11.2; `UNTESTED` in §16/§19. Carries a requirement row in § Non-functional requirements since 2026-08-22 |
...
| `REQ-CLAIM-01` | § Non-functional requirements → § Vision §11.2 claim-boundary requirement, whose criterion cites § Out of scope C | Vision ID adopted unchanged rather than re-minted. Given its own requirement row 2026-08-22 (`GOV-2026-08-22-INC-01` Rec 10); the prohibited-class enumeration remains maintained in § Out of scope C only |
