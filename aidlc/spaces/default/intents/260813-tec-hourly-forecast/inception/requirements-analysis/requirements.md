...
| FR-P1-04-12 | **The permitted ML input space is closed.** The feature set is exactly the TE §6.2 dictionary — no field outside that table, and no derived tensor built from one, enters training or inference. Window length is one frozen value per feature-set ID, shared across all model families | A field absent from the §6.2 dictionary fails feature construction rather than passing silently; the feature manifest enumerates only §6.2 fields; the window length is one recorded value per feature-set ID | [TE §6.2 "This table is the complete permitted ML input space"] [TE §6.4] | `UNTESTED` — dictionary closure has no WS/TA row; candidate new TA row via Vision §15.2 |
