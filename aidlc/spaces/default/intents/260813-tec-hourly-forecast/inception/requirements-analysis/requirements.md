...
**Type and complexity.** New build on a partially populated workspace: two scripts, one notebook, twelve months of derived audit evidence, and — corrected 2026-08-21 — **three test modules that do exist**, `tests/test_acquisition_window.py`, `tests/test_phase_boundary.py` and `tests/test_release_hashes.py`, against which FR-P1-01-5, FR-P1-02-6, FR-P1-03-2, FR-P1-04-11, FR-WS-3 and REQ-NFR-A2 all discharge. Still absent: `src/`, `configs/`, `pyproject.toml`, and the remaining fifteen of REQ-ENG-4's eighteen test modules. The previous flat "no `tests/`" contradicted six of this document's own requirements [origin `IMPL-9`+`DATA-18`+`VAL-10`]. System-wide scope, complex domain, heavy external governance. Depth: Comprehensive.
...
| FR-P1-01-7 | The Canadian F10.7 archive is audited from 2022-03-18 onward for the documented month-long outage; exact missing dates, qualifiers and any reconstructed values are reported. **No imputation, substitution or reconstruction occurs until the measured gap is recorded and governed.** | The audit report exists with exact dates; no filled value exists in the series before the governing decision. **Three further choices are frozen in `features.yaml` under a D-number before G-05, none of them cosmetic:** (a) **which of the three daily readings is the daily value** — the contract has never stated it; (b) the **duplicate-UT-timestamp tie-break**, five such days existing in 2022; (c) the **high-spread handling** for the four days whose within-day spread exceeds 20% of the median, **three of those four contaminated readings falling at 20 UT, the conventional pick**. A run whose `features.yaml` leaves any of the three unset fails the zero-TBD preflight rather than resolving it by convention. **Sequenced behind REQ-ENG-1, not overdue:** no `configs/` directory exists yet, so the three freezes are due when `features.yaml` is created and before G-05, on the same footing as the D-19 thresholds in FR-P1-03-5 [dependency stated per `DATA-24`] | [intent obligation 2] [TC-20] [EC1-AUDIT] [EC1-R-2, EC1-R-3, due G-04 before G-05] [origin `DATA-15`] | `UNTESTED` — the outage audit has already run at zero missing dates; the three selection choices have no WS/TA row and are recorded here as live residuals rather than as discharged |
...
| 12 | **FR-WS-1 asserted both "single-station" and "all three cells" for the same `plumbing_7day` fixture.** TE §15.1 mandates one station and TC-03f describes the plumbing fixture as single-station; D-11 froze the window across all three cells. Both statements sat in one sentence with the conflict recorded nowhere | **No reading is adopted.** The conflict is between a frozen decision (D-11, student-owned under Q-31) and an authority sentence (TE §15.1), and resolving it either way changes what the fixture *is*: narrowing D-11 to one station would re-open a closed freeze, and reading §15.1 as permissive would rewrite an authority clause by inference. It is put to the supervisor as stated | **Open, and blocking for the plumbing fixture's manifest** — `fixture_manifest.yaml` cannot state its identity while the station count is contested. Recorded 2026-08-21 per `GOV-2026-08-20-RA-01` `DATA-11`+`IMPL-5` |
...
FR-P1-04-12, FR-P1-04-13, FR-P1-04-14, FR-P1-04-15, **FR-P1-04-16**,
**FR-P1-04-17**, **FR-P1-04-18**, FR-P1-05-3, **FR-P1-05-4**, FR-P1-05-5,
FR-P1-05-6, FR-P1-05-7, FR-P1-05-14, FR-P1-05-15, FR-P1-05-16, FR-P1-05-17,
FR-P1-05-18, FR-P1-05-19, **FR-P1-05-20**, **FR-P1-05-21**, **FR-P1-05-22**,
FR-WS-2, FR-WS-3, REQ-CLAIM-01, REQ-NFR-A1, REQ-NFR-A2, **FR-P1-02-7** and **FR-P1-02-8** — **40 fully untested requirements**. No row is partially untested any more: FR-P1-02-1 held both a test row and an `UNTESTED` qualifier in one pass/fail cell until the fifth revision split its §6.2 content out as FR-P1-02-7, so every requirement now carries exactly one verdict [per `DATA-23`].
