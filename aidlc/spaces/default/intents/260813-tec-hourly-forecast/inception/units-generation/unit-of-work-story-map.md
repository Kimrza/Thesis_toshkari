...
**Nine** of these carry a blocker ID (BLK-01 through **BLK-09**); **BLK-01 is closed** and its row is
retained as a closed row so the record is not lost. *(Corrected 2026-08-23 from "Seven … BLK-07":
**BLK-08** and **BLK-09** were registered on 2026-08-23 under the owner's Q10 = A ruling, and this
span sentence was not extended with them. Count derived from `unit-of-work.md`'s `### BLK-0…`
headings — nine, with no gap or duplicate — rather than carried.)* `unit-of-work.md` § Blocker register holds the
...
| ~~`three_seed_mean` has no frozen-seed-set parameter, so it can only be implemented by inlining `{1337, 2024, 7}` (forbidden) or by a weaker distinctness check~~ — **mechanism limb resolved 2026-08-23**: the signature now takes `expected_seeds: frozenset[int]` from `ConfigSnapshot.seeds`, exactly as this blocker's Required-resolution field specified. The **contract** limbs (input/output types, alignment, allowed partitions, failure conditions) stay open | **BLK-03** | `models-and-baselines`, via `functional-design`; **seed values closed 2026-08-22** (D-122) | Open **on the contract limb**; an **exit** condition on stage 3.1. D-122's sign-off closed 2026-08-22 with values verified unchanged (development seed 42 and final seeds {1337, 2024, 7}. The bootstrap seed **20221201** is frozen separately by TE §13.6 / TC-19 (Q-27) and is **not** part of D-122’s item set — attribution corrected 2026-08-22 per `GOV-2026-08-22-UG-02` Rec 11) — **authority only: the values reach `three_seed_mean` from `ConfigSnapshot.seeds` via `configs/seeds.yaml` and must never be inlined in `src/models` or any other implementation file** (TC-03e). Carried from the advisory review of `component-methods.md`. Reaches `evaluation-and-comparison`, `statistical-inference` and `regimes-diagnostics-reporting` downstream. |
| ~~`fit_transforms` types `train` as an unconstrained DataFrame, so the full-dataset fit remains representable~~ — **mechanism replaced 2026-08-23 by ADR-11**: `fit_transforms(bundle, *, partition)` compares the bundle's scored range against the partition's training range, `apply_transforms` is removed, and the cross-partition check is an **identity** comparison with one enumerated `REFIT` → `DEC` `score` exception. The **contract** limbs and the enumerated negative control stay open | **BLK-04** | `features-and-splits`, via `functional-design`; leakage evidence Supervisor at G-04/G-05 | Open. **Exit condition on stage 3.1, not an entry condition** (`GOV-2026-08-22-REM-01` Rec 2): the affected units **may enter** 3.1, **may not complete or exit** it without the approved contract, and **no implementation may proceed** while it stands. **⚠ The safeguard's mechanism changed and this row said otherwise until 2026-08-23.** Superseded text, preserved: *"per-fold train-only fitting on the named fold's training partition only, with a `LeakageError` when `train`'s index is not a subset of that partition"* — that is **containment**, which ADR-11 rejected after stage 3.1 spent five review cycles on it, because the training ranges nest and F4's transform on April passes every containment test while F4's fit saw April. Rewritten under the owner's ruling Q8 = A, discharging the obligation `application-design` deferred here. (This row carried no entry/exit statement until 2026-08-22, when the ruling reached the register and the per-unit lines but not this table; corrected per `GOV-2026-08-22-UG-02` Rec 1 — the same partial-sweep pattern that left the containment wording standing here for a further day.) Carried from the advisory review of `component-methods.md` and ADR-01. Reaches every unit downstream of features, since every reported number inherits the fit — including `fixtures-and-reproducibility`, whose clean-run tolerance comparison and TA-21 traceability matrix consume those artifacts. BLK-03 reaches it the same way. |
| `tests/test_prepared_target_schema.py` is mandated by the §12 tree but named by **no** design surface — *(premise rewritten 2026-08-23 under the owner's ruling Q11 = A; it read "The D-17 target-schema test implied by FR-P1-03-5 has no module name and no §12 tree entry", and both halves of that are now false)* | **BLK-05** | **Owning design surface: `functional-design` (3.1) for `target-standardization`.** No further §12 amendment, and therefore no §15.2 change record, is required — that was the superseded premise | **Naming and documentation limbs RESOLVED 2026-08-22 — `tests/test_prepared_target_schema.py`** (`CR-2026-08-22-TARGET-SCHEMA-TEST`). **Owning-design-surface limb OPENED 2026-08-23**: `application-design`'s advisory review found the module named in no module, package or dependency inventory across the five 2.6 artifacts. **Implementation and execution PENDING**: the module does not exist and has never been run. FR-P1-03-5 remains untested — naming a module is not adding an acceptance row, and having a tree entry is not having an owner. |
| `Transform.inverse` is specified as reachable from `Prediction.transform_id`, a `str`, with no lookup, registry or import edge named — so `ABL-DIFF`'s obligation to inverse-transform to absolute TECU before any metric has **no executable path** | **BLK-08** | `evaluation-and-comparison` (owner) with `features-and-splits` (co-owner, since `Transform` and its fitted state live there), via `functional-design` | **Registered 2026-08-23** under the owner's ruling Q10 = A, carried from `application-design`'s gate as accepted risk. Open; exit condition on 3.1 for both owning units. **Blocks a reported quantity, not an internal detail** — every number the thesis reports is in TECU. Reaches `statistical-inference` and `regimes-diagnostics-reporting` downstream. |
| `Partition` carries no `train_start`, so *"the partition's training range"* — the value two of ADR-11's newly-added raises compare against — is reachable only by an unwritten January-1 convention | **BLK-09** | `features-and-splits`, via `functional-design` | **Registered 2026-08-23** under the owner's ruling Q10 = A. Open; exit condition on 3.1. Deriving the range from `train_end` and a hard-coded year is **not** available — that puts a scientific constant in source, which TC-03e forbids. Reaches the four units that inherit the fit. |
...
| **36** requirements carry no §16/§19 row *(corrected 2026-08-23 from **40**; superseded 2026-08-22 when TA-33…TA-36 were approved under `CR-2026-08-22-LEAKAGE-TA`, which this same artifact records at § Coverage — the row and the section it belonged to disagreed for two days)* | — | stages 3.1 and 3.2 | Open by design; enumerated per unit above. |
