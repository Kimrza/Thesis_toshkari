# Team Practices — Hourly VTEC Forecasting (TEC_Project Phase 1 onward)

Affirmed at the practices-discovery interview (`practices-discovery-questions.md`,
Q1–Q15, FU-1, FU-2, all human-answered and confirmed "Looks correct"). These five
sections are settled team practice, promoted into `team.md` at the affirmation
gate — not a proposal awaiting confirmation.
...
- `PreFlight/Technical_Environment_and_Research_Implementation(1)(2).md` —
  §1.1 (freeze-gate obligation), §1.3 (v2.0 change row; script/notebook/config
  counts — see the unresolved contradiction noted in `evidence.md`), §6.2
  (feature dictionary, lag rules, IRI-denial binding rule), §7 (`src/`
  ownership, quoted), §7.0 (Phase 1 hard prohibition, `test_phase_boundary.py`,
  P1-00–P1-06 stage table), §7.0B (phase transition freeze), §8.1 (approved
  stack, Python 3.11, pytest, TensorFlow pin open), §8.3 (prohibited-stack
  deny-list, platform governance), §9.1 (platform roles), §9.2 (walking-skeleton
  sequencing, CPU posture), §10 (credential handling, driver-provenance
  controls, Madrigal identity requirement), §10.1 (reuse register), §11
  (NFR-IRI-01, NFR-LEAK-01, NFR-FAIR-01, NFR-REP-01, NFR-DET-01,
  NFR-PHASE-01, NFR-SEC-01, NFR-LIC-01, NFR-AUD-01), §12 (repository tree,
  `tests/` 17-module list, import-boundary rule, script/notebook naming),
  §13.1 (environment lock, per-run capture list, code commit), §13.2 (ordered
  clean-run contract), §13.3 (immutable dataset release manifest — corrects the
  earlier §6.13 miscitation), §13.4 (registry integrity, `locked_test_accessed`,
  `code_commit`), §14 (notebook production-logic prohibition), §16 (WS-01–WS-20
  acceptance checklist), §16.1 (WS-01–WS-08 assigned to G-P3A), §18.2
  (forbidden-choice table, absolute no-post-hoc-change rule), §18.3 (preflight
  gate, ten named gate tests), §19 (TA-01–TA-32 approval checklist).
- `PreFlight/vision_document(3)(2)(2).md` — §2.2 (Phase Boundary), §2.4
  (comparison hierarchy, binding honesty rule), §5.3–§5.4 (locked evaluation,
  practical-relevance reference), §6 (feature contract), §7.1 (IRI-free
  boundary), §8.3 (required pre-G-05 December coverage audit; Colab/Drive
  removed as governed platforms), §9.2 (CPU-complete path), §13.1 (gate table,
  including G-05, G-06, G-07, G-P2, G-P3A, G-P3C).
- `evidence/DECISIONS.md` — D-1 through D-10 and the signature table, as the
  project's actual unit of auditable, recorded scientific change.
- `scripts/audit_ec1_drivers.py`, `scripts/merge_coverage_year.py`,
  `notebooks/madrigal_phase1_coverage_audit.ipynb` — directly read for code
  style, structure, and the frozen scientific values in the notebook's cell 4.
- `aidlc/spaces/default/intents/260813-tec-hourly-forecast/ideation/feasibility/constraint-register.md` —
  TC-03c, TC-03d, TC-03e, TC-03f, TC-06, TC-15, TC-21, OC-03.
- `.claude/scopes/aidlc-research-pipeline-governed.md` — `skeleton: off`
  frontmatter and rationale; `3.7 (ci-pipeline)` and `infrastructure-design`
  listed among stages to skip.
- `aidlc/spaces/default/intents/260813-tec-hourly-forecast/aidlc-state.md` —
  `Languages: TypeScript` field, checked against actual observed code;
  `Scope Configuration`, `user-stories` (2.4) marked `SKIP`.
- `aidlc/spaces/default/memory/org.md`, `project.md` — the framework's
  suggested defaults and the project-level rules governing gate inputs,
  assumption-splitting, and non-relocation of normative-core requirements.
- `sensors/aidlc-coverage-threshold.md` — embedded default targets (line 80 /
  branch 70), advisory severity.
- `governance/reviews/GOV-2026-08-15-FE-01.md` — finding `GOV-F-06`.
- `practices-discovery-questions.md` — Q1–Q15, FU-1, FU-2, all human-confirmed.
- `contributions/aidlc-quality-agent.md`, `contributions/aidlc-developer-agent.md`,
  `contributions/aidlc-devsecops-agent.md` — support review findings integrated
  throughout this file; the specific finding cited inline where it drove a
  correction.

## Assumptions & Open Questions

- The **seven-day plumbing fixture window is now stamped**: D-11 in
  `evidence/DECISIONS.md` (2026-08-16), November 2022, 2022-11-01 to 2022-11-07,
  all three cells, student sole-signed under Q-31. The **one-month all-station
  scientific window remains open**; 2022-04 and 2022-07 are ineligible until the
  `raw_isprint_cache/` re-acquisition, and 2022-12 is excluded as the locked
  month. Owner: student. (This bullet previously said no month was stamped and
  that the check was a pending follow-on; superseded by D-11.)
- Supervisor countersignature status, updated 2026-08-16, carried explicitly
  rather than silently assumed either way:
  - **Countersigned 2026-08-16, recorded on the student's report:** (1) the
    WS-09–WS-20 Phase 1 acceptance-set reading, WS-01–WS-08 deferred to G-P3A
    (Q6=A); (2) the addition of `tests/test_acquisition_window.py` as a new module
    amending §12's exhaustive `tests/` tree.
  - **Still open:** (3) the §12/§14/§19 (nine scripts, five notebooks) reading
    over §1.3's stale "Scripts 18 → 7 / Notebooks 11 → 4" change-log row (Q15=A),
    noting that Vision §14.2 D-130 already records the seven/four counts as
    superseded though its pointers carry no counts of their own; (4) the missing
    D-3/D-144 signature artifact and its two still-unfrozen sub-values, the
    coordinate-to-cell rule and the numerical coverage minimum.
  All four are set out in `governance/COUNTERSIGNATURE_REQUEST_2026-08-16.md` and
  in `evidence.md`, split from this project's own verification obligations per
  `project.md` § Way of Working.
- Whether the AGPLv3 Global-TEC-forecasting repository's distribution
  obligations permit the direct copying §10.1 otherwise approves is a
  governance dependency owned outside this project (student + supervisor +
  institutional policy per §18.2), not a practice this stage can settle — left
  open and named as such in `evidence.md`.
- The TensorFlow pin remains explicitly unresolved in the governing document
  itself (§8.1: frozen only after Kaggle/local fixture installation passes) and
  is not treated here as decided; it stays with the other `TBD — freeze gate`
  items in `discovered-rules.md` § Assumptions & Open Questions.
