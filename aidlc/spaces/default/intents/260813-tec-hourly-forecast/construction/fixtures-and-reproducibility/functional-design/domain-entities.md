...
- **G-09 is not signed.** These shapes are design only: no module, manifest, receipt, gate result, evidence emitter, test module or `tests/fixtures/` directory is created. **TE §18.3's stop-and-report rule binds** while any P0 decision is unresolved.
