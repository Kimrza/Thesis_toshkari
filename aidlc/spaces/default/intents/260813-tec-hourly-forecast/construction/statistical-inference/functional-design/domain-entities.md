...
- `../foundation/functional-design/business-rules.md` — R-01: the ~~fourteen~~ ⚠ **FIFTEEN**-exception `IntegrityError` hierarchy *(corrected 2026-08-30 on adversarial finding 2, Major: this citation carried the pre-amendment count unqualified while § 8's operative content already said fifteen)* — R-01 was amended 2026-08-28 with **`PartitionError` promoted as the fifteenth**, the count restated as **derived and printed**, and `InverseTransformError` **explicitly disposed** under the *"any future integrity-related exception"* clause rather than admitted as a sixteenth — its `src/data/config.py` base, **`BootstrapError` named among those raised by other units**, and the constructor contract (file or resource + violated expectation).
