...
- `../foundation/functional-design/business-rules.md` — R-01 (the ~~fourteen~~ ⚠ **FIFTEEN**-exception `IntegrityError` hierarchy — *corrected 2026-08-30 on adversarial finding 2, Major: this citation carried the pre-amendment count unqualified while this file's own operative content (W-1) already said fifteen*. R-01 was amended on 2026-08-28: **`PartitionError` promoted as the fifteenth**, the count restated as **derived and printed** rather than carried in prose, and `InverseTransformError` **explicitly disposed** under the *"any future integrity-related exception"* clause rather than admitted as a sixteenth. **`BootstrapError` is still named among those raised by other units**, and this unit's own raises are unaffected), R-10 and § Stage entry contract (the six ordered steps `07` performs before this unit runs).
...
**Verdict:** READY
**Reviewer:** aidlc-architecture-reviewer-agent
**Date:** 2026-08-30T12:13:15Z
**Iteration:** 1 (fresh budget after human gate rejection of the prior READY)

### Findings

| # | Severity | Location | Finding | Recommendation |
|---|---|---|---|---|
| — | — | — | No surviving Critical or Major finding. The two Major findings from the prior adversarial pass are re-verified below: one fixed in the design artifacts, the other correctly left unfixed because fixing it would violate a standing project rule. | — |

### Findings from the prior pass, re-verified

**Finding 2 (prior Major) — CONFIRMED FIXED.** The prior pass found `business-logic-model.md` Sources line 84 and `domain-entities.md` Sources line 67 each carrying an unqualified "fourteen-exception `IntegrityError` hierarchy" claim contradicting each file's own operative content. Re-read both lines directly: `business-logic-model.md:84` now reads "R-01 (the ~~fourteen~~ ⚠ **FIFTEEN**-exception `IntegrityError` hierarchy — *corrected 2026-08-30 on adversarial finding 2, Major* …)" and `domain-entities.md:67` carries the identical correction, both dated and both restating the amendment substance (`PartitionError` promoted as the fifteenth, the count now derived-and-printed, `InverseTransformError` explicitly disposed under the any-future-exception clause, `BootstrapError` still named among those raised by other units). A full `grep -n -i fourteen` sweep of all three artifacts under review (business-logic-model.md, business-rules.md, domain-entities.md — `functional-design-questions.md` excluded per scope) turns up zero remaining unqualified "fourteen" assertions: every hit is either struck through with a FIFTEEN correction, carries the "as written… not the live enumeration" qualifier (`business-rules.md:92,98`; `domain-entities.md:58`), or sits inside an already-dated `⚠ SWEPT … SUPERSEDED` box. A separate sweep of fenced ```mermaid``` blocks across the three files for `14`/`fourteen` returns no hits, and a sweep for `not on disk yet` / `still reads` / `has not yet written` returns only instances already wrapped in a SWEPT/SUPERSEDED correction box. Finding 2 does not survive.

**Finding 1 (prior Major) — CORRECTLY NOT FIXED; not grounds for NOT-READY here.** `functional-design-questions.md` lines 57 and 551 still assert "fourteen" unqualified, and the Consolidated Summary Confirmation at line 592 still claims a "three sites" sweep that these two lines contradict — verified unchanged by direct read. But `functional-design-questions.md` is a receipted, human-signed record; `governance/CHANGE_RECORD_PROCEDURE.md` permits annotate-in-place only with owner approval for the specific item, and `project.md`'s learned correction (`fd-2026-08-30-never-edit-signed-record`) states this exactly: never edit a signed record to match a later derivation — route the correction to the artifacts the unit owns and put one explicit ruling to the human. The dispatch brief for this pass instructs the same: assess only whether the design artifacts state the fact correctly, and treat the questions-file discrepancy as gate input. Since the three artifacts actually under review (`business-logic-model.md`, `business-rules.md`, `domain-entities.md`) state the fact correctly (see Finding 2 above), and the one file that doesn't is out of this review's edit scope by binding project rule, this is not a defect chargeable against the reviewed artifact set. It remains open as gate input: the human should be told the "three sites" self-certification in the signed receipt is false as written (two further unswept instances exist in that same file) and asked to rule between annotate-in-place-with-approval or a correction recorded in the gate record alone.

### R-113 precondition 2 vs. `models-and-baselines` R-92 (dispatch brief's named at-risk item)

Re-verified by direct read of `business-rules.md` R-113 precondition 2: a `partition_id` mismatch raises **`PartitionError`**, matching `models-and-baselines` R-92 exactly; an absent (`None`) stamp of either kind or a `transform_id` mismatch raises `LeakageError`; a mask/member `partition_id` mismatch raises `FairnessError`. The rule's own change-log records this as the Recommendation-8 correction applied 2026-08-28, replacing an earlier import of R-105's taxonomy disagreement. Spot-checked the one named integration point in `foundation` (permitted carve-out — resolved via the shared contract, not by browsing): `foundation/functional-design/business-rules.md:72,178-274` confirms R-01 is amended, `PartitionError` promoted to the fifteenth with a derived-and-printed count, declared in `src/data/config.py`. No taxonomy disagreement survives.

### Bootstrap construction vs. TC-19 / TE §13.6 (tried to break it)

- **Within-station/naive substitution**: not reachable. R-116 enforces one `block_indices` sequence applied to all three stations simultaneously (structurally unrepresentable per-station); the rejected Q-27 method survives only as the quarantined, never-reported W-6/R-120 widening comparator.
- **All-three-stations-together**: R-116 check 2/7 confirmed; ragged coverage is handled by a declared per-station masked-row rule, not by breaking the vector property.
- **10,000 / 20221201 / 95%**: `replicates` and `block_hours` are config-passed, never defaulted (R-118); seed 20221201 sourced from `ConfigSnapshot.seeds`, never inlined, and a call without `seed` is a `TypeError` by signature (R-117); the 95% level is stated in R-113/R-121's citation of TE §13.6 and routed to `experiment.yaml`.
- **Cross-station paired-error correlation**: R-121 computes all three pairwise correlations (ARUC–BSHM, ARUC–NICO, BSHM–NICO) on the estimand's own paired-difference series, carried machine-readably on `BootstrapResult`, never conditioning the widening guard — matches the mandatory-disclosure requirement.
- **Practical-relevance threshold after December opens**: no threshold-setting logic appears anywhere in this unit's rules; out of this unit's scope entirely (owned by `evaluation-and-comparison`/gate), so no drift to find here.
- **Estimand**: R-114/W-2 explicitly reuse only R-108's steps 2–3 on precomputed differences (paired loss differential, benchmark minus model, equal-station weighting) with no second copy of the differencing or weighting arithmetic in this unit.
- **Genuinely-proposed items stay proposed, not silently decided**: grepped every "PROPOSED"/"percentile" occurrence — the interval method (Q7), the correlation series choice (Q9), and the block scheme (Q3, "the fifth entry") all carry live "PROPOSED"/"stated at the gate"/"not settled by this artifact set" framing in both `business-rules.md` and the Gate items list (`functional-design-questions.md` § Gate items, five entries, all confirmed present); none is asserted as frozen.

### Validation Tool Results

No stage-declared validation tooling was listed for this dispatch. Verification performed by: `grep -n -i fourteen` and a fenced-mermaid-block sweep across the three artifacts under review; direct read of `business-logic-model.md:84`, `domain-entities.md:67`, `business-rules.md:92,98`; direct read of `functional-design-questions.md:57,551,592` to re-confirm Finding 1's unfixed (and correctly-unedited) status; direct read of R-113, R-116, R-117, R-118, R-121, and the Gate items section; one permitted spot-check into `foundation/functional-design/business-rules.md` (the named integration point for R-01/`PartitionError`) to confirm the amendment independently rather than trusting this unit's citation of it.
