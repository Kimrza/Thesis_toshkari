...
- ~~**Open — `PartitionError`'s declaration site**~~ ⛔ **CLOSED — RULED 2026-08-28: `src/data/config.py`** (§ 10's dated box). *(Corrected 2026-08-30 on adversarial finding 1, Major: this Open bullet went on asserting the question unresolved while § 10's box already carried the ruling. The Open-items list is the more prominent, grep-discoverable representation, so a reader treating it as authoritative could re-raise a closed question or implement against the pre-ruling `src/models/` default.)* The owner amended the Rec 8 ruling's wording **on exactly the ground this bullet raised**: on the approved dependency matrix a `src/models/` declaration is unreachable from every module that raises it here (`src/features` → `src/models` and `src/data` → `src/models` are both `—`). Declaring it beside `IntegrityError` in `src/data/config.py` needs **no matrix amendment**. `models-and-baselines` remains the **semantic owner** — R-92's discriminating rule unchanged — but is no longer the declaration site. **A recorded ruling, not an open item.**
