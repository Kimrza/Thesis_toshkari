...
- ~~**Open — `PartitionError`'s declaration site**~~ ⛔ **CLOSED — RULED 2026-08-28: `src/data/config.py`.** *(Corrected 2026-08-30 on adversarial finding 1, Major: this Open bullet went on asserting the question unresolved while the `[assumption]` above — in this same file — already carried the ruling. The Open-items list is the more prominent, grep-discoverable representation, so a reader treating it as authoritative could re-raise a closed question at the gate or implement against the pre-ruling `src/models/` default.)* The owner amended the Rec 8 ruling's wording **on exactly the ground this bullet raised**: on the approved dependency matrix a `src/models/` declaration is unreachable from this unit's modules (`src/features` → `src/models` and `src/data` → `src/models` are both `—`, while every raise here lives in `src/data/splits.py` or `src/features/*`). Declaring it beside `IntegrityError` in `src/data/config.py` needs **no matrix amendment**. `models-and-baselines` remains the exception's **semantic owner** — R-92's discriminating rule is unchanged — but is no longer its declaration site. **This is a recorded ruling, not an open item.**
...
> stated (finding 11a). **BLK-04 remains an open exit condition. G-09 remains unsigned. ⚠ **G-09 IS SIGNED as of 2026-08-28 (D-31)** — this prohibition's stated ground no longer holds, and module creation is authorised. **The other grounds stated alongside it, if any, are untouched**, and D-31's disclosure travels with the signature: the §18.3 preflight never ran, the critical tests are unexecuted in this environment, and `aws_ai_dlc_preflight_report` does not exist. **No scientific value becomes fillable** — TE §18.2 and §18.3's stop-and-report rule are unchanged.**
...
> it was raised. **BLK-04 remains an open exit condition. G-09 remains unsigned. ⚠ **G-09 IS SIGNED as of 2026-08-28 (D-31)** — this prohibition's stated ground no longer holds, and module creation is authorised. **The other grounds stated alongside it, if any, are untouched**, and D-31's disclosure travels with the signature: the §18.3 preflight never ran, the critical tests are unexecuted in this environment, and `aws_ai_dlc_preflight_report` does not exist. **No scientific value becomes fillable** — TE §18.2 and §18.3's stop-and-report rule are unchanged.**
