...
| `tests/test_feature_leakage_guards.py` | §12-mandated; **created 2026-09-20** under the approved governance remediation (Recommendation 12), superseding key decision 9's "was NOT built". 21 test functions / 25 collected cases (one function parametrised over 5 mutants), 849 lines. TE §19's four negative-path rows, each pushing a violating input through `build_features` — the boundary TA-36's row fixes by name — and asserting the raise: **TA-33** dictionary closure (invented row; `iri_`-prefixed field; the canonical `iri2016_t_plus_1_tecu` on the unconstrained `target_support` row; REMOVED `ssn`; raw longitude; the window length placed in a grid, in two spellings) plus a positive closure limb over the emitted provenance; **TA-34** the `vtec_lag_*` route onto the driver carry-forward path closed at name/row agreement, the class-table invariant, the `carry_forward` boundary itself, and an incomplete `vtec_seq_24` window shown EXCLUDED AND COUNTED with the count derived from the window length; **TA-35** R-78's rules (no approval → absent from the input set; approval without id / without timestamp; approval after the freeze; `lag_hours: 0`; target-hour quality permanently forbidden) plus the discriminating positive; **TA-36** a value repeated outside its interval, a value shifted one hour, an interpolated value that still satisfies its declared lag (caught on traceability, not on lag), and an AST call-name scan finding no interpolation call on the driver path. Plus the TC-12 consuming-side controls: duplicate driver epoch through `build_features`, and the joined-grid identity guard (direct-call refusal + a spy proving `build_features` invokes it on the POST-JOIN frame). Authors no fixture others reuse; self-contained except `test_split_embargo`'s M10 synthetic snapshot/partitions |
| `tests/test_train_only_transforms.py` | §12-mandated. **31** test functions (was 28; +3 on 2026-09-20 for R-58 limb 3's conservation invariant, now WIRED into `carry_forward` — see § Post-receipt amendment — 2026-09-20): full-dataset fit raises, train-role bundle at evaluation fails, cross-partition transform fails, untransformed consumer fails, strict-subset and over-wide and one-row-off ranges fail, DEC fit fails; the 30-condition M10 enumeration derived in-test (36 ordered pairs − 6 identities = 30; 1 exempt `REFIT`→`DEC`; 29 raise + 1 passes = 30), `PartitionError`-vs-`LeakageError` discrimination, carry-forward boundary, entry-point refusals through `build_features` |
...
9. ~~**`tests/test_feature_leakage_guards.py` (R-76a / TA-36 primary test) was NOT built** — the approved plan's Step 8 does not list it. R-76a's enforcement raise (`AlignmentError`) IS implemented in `build_features` with a control in `test_feature_availability.py`; the module itself remains owed.~~ **SUPERSEDED 2026-09-20.** True when written; no longer true. The module was built under the approved governance remediation of 2026-09-20 (Recommendation 12) and exists on disk with 21 test functions / 25 collected cases, carrying at least one negative control per TA row at the `build_features` boundary. The original text is struck rather than deleted, so a reader of this list sees both what was claimed and what is now the case. R-76a's enforcement raise stays in `build_features` and `test_feature_availability.py` keeps its own control; what moved is that TA-36's PRIMARY rejection test now sits where TE §19 puts it. **TA-33…TA-36 remain `Pending`**: the module's EXISTENCE is not its executed evidence, and no test was run in the remediation session.
...
## Post-receipt amendment — 2026-09-20 (Recommendation 12 + Recommendation 14: the mandated leakage-guard module, and the driver-consuming guards wired)

*Appended under `project.md` `code-generation:gf-3`. Nothing above is rewritten except the
two BODY corrections this amendment requires and names: key decision 9 is struck in place
(it asserted the module "was NOT built"), and the `test_train_only_transforms.py` row's test
count is corrected 28 → 31. Both are corrected in the body, not only here, per
`code-generation:fr-2` — a correction filed only at the foot leaves the stale claim standing
for its own reader. Authority: the project decision owner's approval of the 2026-09-20
governance remediation, Recommendations 12 and 14.*

| Module | What changed (measured, `git diff --numstat` against HEAD `ff5c683`) |
|---|---|
| `tests/test_feature_leakage_guards.py` | **NEW**, 849 lines, untracked. 21 test functions / 25 collected cases. TE §12's mandated module and TE §19's named evidence artifact for TA-33, TA-34, TA-35 and TA-36 — see the § Files created row for the per-row control set. Every control pushes a violating input through `build_features` (TA-36's row: *"the primary rejection test sits at the feature-building enforcement boundary"*) or through the rule's own public entry point where that differs; each carries a `MUTANT:` / `BITES:` docstring naming what it would catch. |
| `src/features/build.py` | +41 / −1. (1) `_hourly_series` refuses a duplicate driver epoch, naming it — the one ingest path that previously let a second row at one epoch win by plain assignment, with the winner then broadcast to every station's row. (2) `build_features` calls `spaceweather.assert_identical_across_cells` per driver field on the post-join frame. (3) `_assert_field_name_clean` widened to reject any name token BEGINNING `iri` — **enforcement that did not previously exist**, see below. |
| `src/features/transforms.py` | +13 / −1. `carry_forward` invokes `spaceweather.assert_carry_forward_conservation` on the series it returns, immediately after `apply_carry_forward`; the `Raises` section documents the new `IntegrityError` and distinguishes it from the class boundary above it (the boundary decides WHO may carry forward; the invariant decides whether what came back is what was recorded). |
| `src/external/spaceweather.py` | +61 / −0, **docstring only, no behaviour change**. New "Boundary split — WHERE each refusal is invoked" section: the driver-PRODUCING guards (`assert_time_indexed_shape`, `assert_grade_eligible`, `assert_single_grade`, `refuse_divergent_rerun`) and the fact that their call site is a NAMED OBLIGATION on the unwritten driver-producing Bolt, not dead code; the driver-CONSUMING guards wired by this amendment; and D-10.1's two-home split stated explicitly against `availability.py`'s inline check. |
| `src/features/availability.py` | +18 / −0, **docstring only, no behaviour change**. The reciprocal half of D-10.1's split: consumption side here (`build_availability_matrix`, `FeatureAvailabilityError`), production side in `spaceweather.assert_single_grade` (`IntegrityError`), each naming the other's function and exception class, with the statement that the two are not redundant and a caller must not catch one expecting the other. `src/evaluation/report_guards.py:9-12` is the precedent. |
| `tests/test_train_only_transforms.py` | +70 / −3. **28 → 31 test functions.** Three for R-58 limb 3's newly wired conservation invariant: an invocation control (a stubbed filler returns a silently filled, unrecorded series and `carry_forward` raises), the guard's own two failure modes kept distinct (unrecorded fill; recorded count that does not reconcile), and a discriminating positive proving the sanctioned bounded carry-forward still passes. Docstring summary updated to match. |

**ENFORCEMENT ADDED — TA-33, `_assert_field_name_clean`.** The IRI name filter tested
`lowered.startswith("iri_") or "iri" in _tokens(name)`. Neither limb matches
`iri2016_t_plus_1_tecu` — this project's OWN canonical IRI field name (TE §6.2 row identity;
`configs/experiment.yaml` `benchmark_b01.output_field`), which does not begin `iri_` and
tokenises to `{iri2016, t, plus, 1, tecu}` with no bare `iri`. The denial mechanism WS-10
exists to prove therefore let through the one name it most needed to catch. Declared on the
`target_support` row — the single branch of `_assert_name_matches_row` that imposes no
name/row agreement — it would have reached the dictionary intact, and with a valid approval
recorded before the freeze it would have entered the model input set. The filter now rejects
any token beginning `iri`; the closed §6.2 row table bounds legitimate names to TE §6.2's
own, none of which begins `iri`, so it cannot over-match. The existing control in
`test_feature_availability.py` injects only `iri_vtec` (the prefix form) and so could not
have found this; the new module's TA-33 parametrisation carries both forms.

**RESIDUAL, recorded rather than worked around.** `build_features` alignment-checks a driver
series only when the producer attached `attrs["observations"]`; a raw frame supplied without
them is admitted unchecked on the own-interval limb. That is `_assert_driver_alignment`'s
stated, pre-existing design ("a series without them is not checked here … never relabelled"),
every current happy-path fixture relies on it, and changing it is a behaviour change outside
this remediation's approved scope. TA-36's rejection therefore depends on the producer
attaching its observations. Carried to the gate.

**Repository state, re-verified at summary-writing time** (`code-generation:c30`): `git log
-1` → `ff5c683`, message still the unedited git commit template (same defect class as
`6246907` / `1670ac8`). **No commit, stage, amend or rebase was performed by this pass** and
none is claimed. `git status --porcelain` reports 39 entries, most of them other units' work
running concurrently in the same tree; the six paths in the table above are this pass's, and
`tests/test_feature_leakage_guards.py` is untracked.

**VERIFICATION STATUS — no test was executed.** No `pytest`, no stage script and no Python
ran in this pass: no usable interpreter exists in this environment, and execution appends to
`evidence/test_run_access_log.jsonl`, a custody artifact. Every claim above is STATIC —
derived by reading the code and counting with `grep`/`git diff --numstat`, never from a run.
`ruff` was not run either. **TA-33, TA-34, TA-35 and TA-36 stay `Pending`**: their evidence
is executed negative-path output captured in a governed run, and the existence of a test
module is not that evidence. The new module and the three wired call sites are UNEXECUTED and
must be run before any claim rests on them.
