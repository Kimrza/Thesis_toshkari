...
**What changed in this unit since the last receipt.** `provenance_class` figures **rebased** with their basis stated (the pre-remediation 9/7/3 were self-invalidated by the parallel `inventory-and-registry` edits); the load-bearing half — `foundation` carries the field **zero** times — restated as the stable fact; **G-09 signed (D-31)**.
