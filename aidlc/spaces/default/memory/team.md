...
- ALWAYS read the credential deny-list obligation in Section Way of Working above as SATISFIED, not owed. That section states the `.gitignore` "needs a credential/secret deny-list it does not currently have (no `.env`, `*.key`, `kaggle.json`, `.netrc`, `credentials*` entries were found - devsecops review, item J)" and calls it a precondition of the first commit. Measured 2026-09-20 on the resumed remediation pass of `GOV-2026-09-20-CG-01` and re-verified 2026-09-21: the deny-list exists at `HEAD`, `.gitignore` lines 63-81, carrying `.env`, `.env.*`, `*.key`, `*.pem`, `*.p12`, `*.pfx`, `*.keystore`, `*.jks`, `kaggle.json`, `.netrc`, `_netrc`, `credentials`, `credentials.*`, `.aws/credentials` and `id_rsa*`, under a header citing Technical Environment Section 10 / NFR-SEC-01 and naming itself the mechanism that exclusion depends on. Every one of the five entries the affirmed practice reported absent is present. The stale claim was carried as escalation 6 of that report's remediation manifest with the note that a memory file is written only through the Section 13 ritual; this correction is that write. It corrects the STATUS of the obligation and changes nothing about the obligation itself - NFR-SEC-01 stands, and the separate, still-open NFR-SEC-01 breach is the historical identity literal in git history (Recommendation 39), which a deny-list cannot reach. Section Way of Working is left standing rather than overwritten, because this path appends and never replaces. (learned 2026-09-21) (learned 2026-09-22) <!-- cid:code-generation:cg-2026-09-21-team-gitignore-denylist-exists -->
