...
| D-28 G-06 locked-test scored set = 2–31 Dec (**29 d, amended** — was disclosed as 30 d until the 2026-09-24 amendment below) | **Yes** | 2026-08-28 | Approved by the project owner under the recorded authority equivalence, at the `functional-design` (3.1) governance gate on `GOV-2026-08-28-FD-01` Rec 6. **Ratifies FU-7 = A (2026-08-26), already built upon by eight units.** Basis is `requirements.md` FR-P1-04-5 + ADR-11's `lead_in_hours` removal; **discloses that Vision §8.2 and TE §7.1 both carry `—` in the Locked-test Embargo column**, so a level-4 paraphrase is the sole textual basis — conflict recorded, not resolved, and carried to G-05. Accepted on three grounds: 1 Dec is furthest from solstice; the bootstrap loses 1 of 31 blocks (conservative); and 720 h divides by 48 where 744 h does not, so the mandatory 48-h sensitivity would have raised under the 31-day reading. **No supervisor signature exists or is claimed.** A revised split manifest is owed at G-05. **AMENDMENT, 2026-09-24 (Student ruling, Layer-2 §2, option B, superseding the originally-drafted option A "bounded 1-Dec read" proposal — that proposal was never implemented and is withdrawn):** `GOV-2026-09-20-CG-01` Recommendation 15 found that the mandatory persistence baselines M-01 (`y(t-1h)`) and M-02 (`y(t-24h)`) cannot forecast the first 24 h of the disclosed 30-day window (2 December) without reading 1 December target history, which lies inside D-28's own 24-hour embargo. Rather than authorize any additional read of the locked month before the one-shot G-06 event (option A, considered and rejected), **the scored window is amended to its true, achievable extent: 3–31 December inclusive, 29 days**, and M-01/M-02 are scored over exactly that reduced set like every other comparison member — no baseline reads 1 December under any circumstance. **Rationale for choosing the zero-additional-contact option:** avoids any further restricted-month access ahead of G-05/G-06; the persistence baselines are not expected to be scientifically sensitive to the loss of one calendar day out of thirty; this is the lower-risk disposition when a real alternative (accept the true count) exists at no scientific cost. ⚠️ **NOT YET CODE-CONSISTENT — measured 2026-09-24, corrected from this amendment's own first-drafted (wrong) claim that the guard was "already in place and unaffected".** Two real conflicts, neither resolved by this documentation-only amendment: (1) `src/evaluation/guards.py:scored_window_statement` derives its disclosed span from `month_start + embargo_hours`, and `embargo_hours` is the ONE global `configs/experiment.yaml` value shared with the Mandated 24-hour fold embargo used by every partition (F1–F4 included, not December-specific) — so the code today still computes and would still ENFORCE "2–31 December, 30 days" as the only valid disclosure for the DEC partition; nothing computes 29 unless a December-specific exclusion mechanism is added, and widening the shared `embargo_hours` to 48 would incorrectly widen every other partition's embargo too. (2) **D-59** (`evidence/DECISIONS.md`, Student+Supervisor countersigned 2026-09-21) explicitly freezes the December day range at **`2022-12-02..2022-12-31`, 30 inclusive days**, "matches D-28's locked scored set exactly (30 days)" — directly contradicted by this amendment — and that value is itself LIVE in `configs/experiment.yaml:376` (`december_day_range: "2022-12-02..2022-12-31"`) and enforced in code by `src/evaluation/regimes.py:read_december_day_range`. **This amendment does not touch D-59 or `configs/experiment.yaml`** — both are supervisor-countersigned frozen values, outside a Student-only ruling's authority to alter, and neither was authorized for edit here. **Consequence: this D-28 amendment currently exists only as a documentation-level scientific ruling; it is not yet implementable without a further decision reconciling D-59 and the `embargo_hours`/December-exclusion mechanism, which needs to be routed back to the owner (and likely the Supervisor, since D-59 carries a supervisor countersignature) before G-05.** **Sweep:** a correction pass across every artifact asserting "30 days" for the scored/locked set is recorded in `governance/CHANGE_RECORD_2026-09-24_d28_29day_amendment.md`, which also carries this conflict in full. |
