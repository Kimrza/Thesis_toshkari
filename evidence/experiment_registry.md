...
| 10 | Full-board governance review of AI-DLC stage 3.1 `functional-design` (`GOV-2026-08-28-FD-01`) — Validation Auditor seat, restricted-root inspection | **Access: 2026-08-28, during the session; exact clock time not captured per command.** **This row created: 2026-08-28, after the read** | `evidence/locked_test_restricted/` — a directory listing (entry names, sizes, modification times); the **key lists** of `audit_evidence_2022-12/sha256_manifest.json` and `audit_evidence_2022-FULL/sha256_manifest.json` (4 entries each); the **key list** of `audit_evidence_2022-12/request_manifest.json` plus one regex test against it | **Nothing computed.** No coverage statistic, no day count, no record count, no hash. No December target value, coverage figure or performance quantity was read, parsed, computed, ranked, plotted or summarised. The 1.6 MB loose extract was **not opened** — filename, size and git status only | No — none exists to inspect | `true` | **Retrospective row, created 2026-08-28. Logged AFTER the read, not before** — the same ordering defect rows 5, 8 and 9 record, and the seat disclosed it in its own pass rather than leaving it to be found. Actor: the AI assistant acting as the Validation Auditor seat, on the project owner's instruction to run the review. **Scope: metadata and manifest-class key lists only**, the class Vision §8.3 permits performance-blind. Authorization: governance verification directed by the project owner. **The pre-read logging obligation was not met, and this row does not repair that** — `open_restricted` does not exist, so no mechanism could have enforced it |
| 11 | Integrity manifest for the loose December raw extract (`GOV-2026-08-28-FD-01` Rec 44a) | **Written BEFORE the read, 2026-08-28** | `evidence/locked_test_restricted/bbox___opt_openmadrigal_madroot_experiments4_2022_gps_31dec22_gps221231g.003.hdf5.txt` — run 2's preserved extract, read **as bytes only** for SHA-256 computation | A SHA-256 digest and the file size, both now recorded in `evidence/locked_test_restricted/loose_artifacts_sha256_manifest.json`: `sha256 3a164af0864b2effde2e527ca190c1b050f5a47179eaffa3ccab770bb366f557`, 1,666,816 bytes. **No field was parsed, no record counted, no value inspected, no statistic computed** — the same scope and method as row 6's post-move hash verification | No — none exists to inspect | `true` | **Written BEFORE the read, as FR-P1-02-3 requires and as rows 6 and 7 set the standard.** Authorised by the project owner, 2026-08-28, on governance report `GOV-2026-08-28-FD-01` Recommendation 44(a): the file is git-tracked, named in neither restricted `sha256_manifest.json`, and was therefore the one December artifact whose mutation would be undetectable. Purpose is custody integrity, not analysis |
| 12 | Custody relocation of `.dst_summary.json` into the guarded evidence tree (**D-30**; `GOV-2026-08-28-FD-01` Rec 44b) | **Written BEFORE the read, 2026-08-28** | `.dst_summary.json` at the repository root — a derived Kyoto Dst summary carrying twelve month keys, the `"12"` entry among them — read **as bytes only** for SHA-256 computation before and after the move, then relocated to `evidence/audit_ec1_2026-08-15/kyoto_dst/.dst_summary.json` | Two SHA-256 digests and the file size, compared for equality to prove the move was byte-identical. **Executed 2026-08-28: `sha256 410927a4ff620b6f7597b18e07746f74233cf5aa87bc84d6f5b0ec25b3e9c064`, 5,653 bytes, identical before and after; the root path no longer exists.** **No field was parsed, no month key read, no day count, storm list or `daily_min` value inspected, no statistic computed** — the same scope and method as rows 6, 7 and 11, and as D-15s relocation | No — none exists to inspect | `true` | **Written BEFORE the read, as FR-P1-02-3 requires and as rows 6, 7 and 11 set the standard.** Authorised by the project owner, 2026-08-28, on `GOV-2026-08-28-FD-01` Recommendation 44(b), recorded as **D-30**. Purpose is custody reachability, not analysis: at the repository root the file sat **outside** R-27s `evidence/` scan root, so the December guard could not reach the one artifact it was designed to catch. `governance-guards` identified the move and expressly declined to perform it without this decision |
