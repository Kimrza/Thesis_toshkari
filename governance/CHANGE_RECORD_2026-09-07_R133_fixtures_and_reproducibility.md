...
| **BLK-08 ↓** | mechanism limb (inverse route) | **CLOSED by D-37 (2026-09-10), reaffirming D-27** — the owner adopted the reaffirmation as drafted and it is recorded in `evidence/DECISIONS.md`; the checked refusal REMAINS the mechanism. (Superseded status, kept as history: "OPEN — resolution drafted, owner adoption owed"; before that, "OPEN".) (updated 2026-09-10: the owner ruled Choice B — affirm D-27's withholding; the reaffirmation is drafted as **D-E** in `governance/CHANGE_RECORD_2026-09-10_owner_rulings_implementation.md` §6 and is NOT adopted. D-27 stands unreopened; `evidence/DECISIONS.md` still ends at D-32) | unchanged: a **checked refusal** — a `toleranced` ledger entry declaring TECU units for an output whose `producing_path` carries no `inverse_route` is refused by the manifest loader (R-139 control 25, at full strength). Under D-E the citable route for the primary path is the IDENTITY route (`identity (D-27: primary target untransformed)`) and `ABL-DIFF` keeps the only real inverse |
