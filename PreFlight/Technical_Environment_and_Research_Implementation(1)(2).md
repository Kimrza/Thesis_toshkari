...
**Tree amendment provenance (added 2026-08-22, change record `CR-2026-08-22-TE-AMEND`; count corrected 2026-08-22 under `CR-2026-08-22-INC-CORRECTIONS`).** The entries listed in the table below were added by amendment rather than in the original tree. They fall into **two distinct authority classes**, recorded separately because they were approved at different times by different acts. The table itself is the enumeration: no total is stated here, because two further amendments the same day (`CR-2026-08-22-TARGET-SCHEMA-TEST`, `CR-2026-08-22-LEAKAGE-TA`) each added a row after the original count was written, and a restated total goes stale on the next amendment. Superseded text, preserved for the audit trail: *"Five entries above were added by amendment rather than in the original tree."* That figure counted only the `CR-2026-08-22-TE-AMEND` additions and was short by the two rows the later same-day records added.
...
Authority to create a module is **not** authority to write it: the table above carries **six newly approved entries, five of them modules** — `src/data/config.py`, `src/data/locked_test.py`, `tests/test_determinism.py`, `tests/test_prepared_target_schema.py` and `tests/test_feature_leakage_guards.py`; the sixth newly approved entry is `PYTHONHASHSEED` in §13.2, an environment-variable setting rather than a module. None of the five modules exists, and each remains subject to its own stage and to gate **G-09** before `code-generation` may create it. `tests/test_acquisition_window.py` is the seventh table row and is **not** in the newly approved class: it was countersigned 2026-08-16 and only its application to this document is new. Superseded text, preserved for the audit trail: *"none of the six newly approved modules exists"* — which counted an environment variable among the modules. Corrected 2026-08-22 under `CR-2026-08-22-INC-CORRECTIONS`.
