...
function handlePracticesPromote(args: string[]): void {
  ...
  // A rule is a MARKDOWN BULLET, not a line. Draft rules wrap across several
  // lines, so splitting on "\n" produced one "rule" per physical line: each
  // fragment got its own `(affirmed ...)` stamp mid-sentence, and a fragment
  // that happened to duplicate an earlier one was silently dropped, truncating
  // the rule. Group each bullet with its continuation lines and collapse the
  // wrap into a single line before stamping.
  const parseRules = (sectionContent: string): string[] => {
    ...
    const flush = () => {
      ...
      if (joined.length > 0) rules.push(joined);
      current = [];
    };
    for (const rawLine of sectionContent.split("\n")) {
      const line = rawLine.trim();
      if (line.length === 0 || line.startsWith("<!--") || line.startsWith("#")) {
        // Blank lines and comments end the current bullet but are not rules.
        flush();
        continue;
      }
      if (/^[-*]\s+/.test(line)) {
        // A new bullet starts here; the previous one is complete.
        flush();
        current.push(line);
        continue;
      }
      if (current.length > 0) {
        // Continuation of the bullet in progress.
        current.push(line);
        continue;
      }
      // Prose outside any bullet: treat as a standalone rule, as before.
      rules.push(line);
    }
    flush();
    return rules;
  ...
  // Re-promotion must be able to repair a previous bad write, not append beside
  // it. Drop any line already stamped with TODAY's date from the two guardrail
  // sections before appending; earlier affirmation dates are left untouched.
  const stripTodaysStamps = (md: string): string => {
    const suffix = `(affirmed ${today})`;
    const lines = md.split("\n");
    let inTargetSection = false;
    const kept: string[] = [];
    for (const line of lines) {
      if (line.startsWith("## ")) {
        inTargetSection = line.trim() === "## Mandated" || line.trim() === "## Forbidden";
        kept.push(line);
        continue;
      }
      if (inTargetSection && line.trimEnd().endsWith(suffix)) continue;
      kept.push(line);
    }
    return kept.join("\n");
  };
  ...
  let newGuardrailsMd = stripTodaysStamps(guardrailsMd);
